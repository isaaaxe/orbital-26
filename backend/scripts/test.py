...
class Result:
    __slots__ = ("suite", "label", "inp", "expected", "actual", "passed", "notes")
    def __init__(self, suite, label, inp, expected):
        self.suite, self.label, self.inp = suite, label, str(inp)
        self.expected = "" if expected is None else str(expected)
        self.actual, self.passed, self.notes = "", True, []

RESULTS = []
_current = None

@contextmanager
def check(suite, label, inp="", expected=None):
    global _current
    r = Result(suite, label, inp, expected)
    RESULTS.append(r)
    _current = r
    try:
        yield r
...
def fail(msg):
    if _current is not None:
        ...
        _current.notes.append(msg)
    print(msg)  # keep live logging
...
async def test_health_endpoint():
    async with httpx.AsyncClient(base_url=base_url, timeout=60) as client:
        ...
        if r.status_code != 200:
            fail(f"health: status {r.status_code}, expected 200")
        elif r.json() != {"status": "ok"}:
            fail(f"health: body {r.json()}, expected {{'status': 'ok'}}")
...
async def test_location_endpoints():
    async with httpx.AsyncClient(base_url=base_url, timeout=60) as client, AsyncSessionLocal() as session:
        ...
        for case in test_input.search_locations:
            with check("locations", f"test '{case.query}'"):
                r = await client.get("/locations/search", params={"q": case.query})

                if r.status_code != case.expected_status:
                    fail(f"{case.query}: status {r.status_code}, expected {case.expected_status}")
                    continue
                ...
                if got_ids != case.expected_ids:    
                    fail(f"{case.query}: ids {got_ids}, expected {case.expected_ids}")

                for row in body:
                    result = await session.execute(select(Location).where(Location.id == row["id"]))
                    db_row = result.scalar_one_or_none()
                    if db_row is None:
                        fail(f"{case.query}: returned id {row['id']} not in DB")
                    elif row["name"] != db_row.name:
                        fail(f"{case.query}: name mismatch for {row['id']}")

        #test locations /get location
        for case in test_input.get_location:
            with check("locations", f"test '{case.location_id}'"):
                r = await client.get(f"/locations/{case.location_id}")

                if r.status_code != case.expected_status:
                    fail(f"{case.location_id}: status {r.status_code}, expected {case.expected_status}")
                    continue
                ...
                if body["id"] != case.location_id:
                    fail(f"{case.location_id}: returned wrong id {body['id']}")

                result = await session.execute(select(Location).where(Location.id == body["id"]))
                db_row = result.scalar_one_or_none()
                if db_row is None:
                    fail(f"{case.location_id}: returned id not in DB")
                elif body["name"] != db_row.name:
                    fail(f"{case.location_id}: name mismatch")
        
        #test get_location_by_type
        for case in test_input.get_location_by_type:
            with check("locations", f"test '{case.location_type}'"):
                r = await client.get("/locations", params={"location_type": case.location_type})

                if r.status_code != case.expected_status:
                    fail(f"{case.location_type}: status {r.status_code}, expected {case.expected_status}")
                    continue
                ...
                if got_ids != case.expected_ids:     
                    fail(f"{case.location_type}: ids {got_ids}, expected {case.expected_ids}")

                for row in body:
                    result = await session.execute(select(Location).where(Location.id == row["id"]))
                    db_row = result.scalar_one_or_none()
                    if db_row is None:
                        fail(f"{case.location_type}: returned id {row['id']} not in DB")
                    elif row["name"] != db_row.name:
                        fail(f"{case.location_type}: name mismatch for {row['id']}")
...
async def test_bus_endpoints():
    async with httpx.AsyncClient(base_url=base_url, timeout=60) as client, AsyncSessionLocal() as session:
        ...
        for case in test_input.get_bus:
            with check("buses", f"test '{case.bus_number}'"):
                r = await client.get("/bus-service/bus", params={"bus_number": case.bus_number})
                if r.status_code != case.expected_status:
                    fail(f"{case.bus_number}: status {r.status_code}, expected {case.expected_status}")
                    continue
                if r.status_code == 404:
                    continue
                body = r.json()
                ...
                if body["bus_number"] != case.bus_number:
                    fail(f"{case.bus_number}: returned wrong bus_number {body['bus_number']}")
                result = await session.execute(select(Bus).where(Bus.bus_number == case.bus_number))
                if result.scalar_one_or_none() is None:
                    fail(f"{case.bus_number}: returned a bus not in DB")

        # test get bus by id
        for case in test_input.get_bus_stop_by_id:
            with check("buses", f"test '{case.bus_stop_id}'"):
                r = await client.get(f"/bus-service/bus_stop/id/{case.bus_stop_id}")
                if r.status_code != case.expected_status:
                    fail(f"{case.bus_stop_id}: status {r.status_code}, expected {case.expected_status}")
                    continue
                if r.status_code == 404:
                    continue
                body = r.json()
                ...
                db_row = result.scalar_one_or_none()
                if db_row is None:
                    fail(f"{case.bus_stop_id}: returned a stop not in DB")
                elif body["name"] != db_row.name:
                    fail(f"{case.bus_stop_id}: name mismatch")
        ...
        for case in test_input.get_bus_stop_by_name:
            with check("buses", f"test '{case.query}'"):
                ...
                if case.query.lower() not in body["name"].lower():
                    fail(f"{case.query}: returned name '{body['name']}' doesn't contain query")
                result = await session.execute(
                    select(Bus_Stop).where(Bus_Stop.bus_stop_id == body["bus_stop_id"]))
                if result.scalar_one_or_none() is None:
                    fail(f"{case.query}: returned stop id not in DB")

        #test get all bus stops 
        with check("buses", f"test all bus stops"):
            r = await client.get("/bus-service/bus_stop/all")
            if r.status_code != 200:
                fail(f"bus_stop/all: status {r.status_code}, expected 200")
            else:
                ...
                if got_ids != expected_ids:
                    fail(f"bus_stop/all: id-set mismatch, "
...
async def test_campus_map_endpoints():
    async with httpx.AsyncClient(base_url=base_url, timeout=60) as client, AsyncSessionLocal() as session:
        ...
        for case in test_input.get_node:
            with check("campus_map", f"test '{case.node_id}'"):
                r = await client.get(f"/campus-map/nodes/{case.node_id}")
                if r.status_code != case.expected_status:
                    fail(f"{case.node_id}: status {r.status_code}, expected {case.expected_status}")
                    continue
                if r.status_code == 404:
                    continue
                body = r.json()
        ...
        with check("campus_map", f"test all buildings"):
            r = await client.get("/campus-map/buildings/all")
            if r.status_code != 200:
                fail(f"buildings/all: status {r.status_code}, expected 200 "
        ...
        for case in test_input.get_building:
            with check("campus_map", f"test '{case.building_id}'"):
                ...
                if db is None:
                    fail(f"{case.building_id}: returned building not in DB")
                elif body["display_name"] != db.display_name or body["building_code"] != db.building_code:
                    fail(f"{case.building_id}: display_name/building_code mismatch")

        # test get building floors by building id 
        for case in test_input.get_building_floors:
            with check("campus_map", f"test get all floors for building id: '{case.building_id}'"):
                r = await client.get(f"/campus-map/buildings/{case.building_id}/floors")
                if r.status_code != case.expected_status:
                    fail(f"{case.building_id} floors: status {r.status_code}, expected {case.expected_status}")
                    continue
                if r.status_code == 404:
                    continue
                for floor in r.json():
                    if not floor["image_url"].startswith(SUPABASE_FLOORS):
                        fail(f"{case.building_id} floor {floor['floor_number']}: "
                    ...
                    if (await session.execute(select(Floor).where(
                        ...
                        fail(f"{case.building_id} floor {floor['floor_number']}: not in DB")

        # test get floor by building id and floor number
        for case in test_input.get_floor:
            with check("campus_map", f"test get floor '{case.floor_number}' for building id: '{case.building_id}'"):
                r = await client.get(
                    f"/campus-map/buildings/{case.building_id}/floors/{case.floor_number}")
                if r.status_code != case.expected_status:
                    fail(f"{case.building_id}/{case.floor_number}: status {r.status_code}, expected {case.expected_status}")
                    continue
                if r.status_code == 404:
                    continue
                body = r.json()
                ...
                if db is None:
                    fail(f"{case.building_id}/{case.floor_number}: returned floor not in DB")
                else:
                    if body["floor_name"] != db.floor_name:
                        fail(f"{case.building_id}/{case.floor_number}: floor_name mismatch")
                    if (body["image_width"], body["image_height"]) != (db.image_width, db.image_height):
                        fail(f"{case.building_id}/{case.floor_number}: image dimensions mismatch")
                    if not body["image_url"].startswith(SUPABASE_FLOORS):
                        fail(f"{case.building_id}/{case.floor_number}: image_url not Supabase -> {body['image_url']}")
    ...
    async with httpx.AsyncClient(base_url=base_url, timeout=60) as client, AsyncSessionLocal() as session:
        try:
            ...
            if r.status_code != 200 or r.json()["username"] != new_username:
                fail(f"update username: got {r.status_code} / {r.json()}")
            await session.commit() 
            db = (await session.execute(select(User).where(User.user_id == user_id))).scalar_one_or_none()
            if db is None or db.username != new_username:
                fail("update: DB username not changed")
            ...
            if r.status_code != 200:
                fail(f"update password: got {r.status_code}")
            await session.commit() 
            db = (await session.execute(select(User).where(User.user_id == user_id))).scalar_one_or_none()
            if db is None:
                fail("update password: user not in DB")
            elif not verify_password(new_pw, db.hashed_password):
                fail("update: DB password not changed")
            if db.hashed_password == old_hash:
                fail("update password: hash unchanged")
...
async def make_test_user(client, session):
    ...
    if r.status_code != 200:
        fail(f"setup: create failed {r.status_code}"); return None, None
    user_id = r.json()["user_id"]
    r = await client.post("/auth/login", data={"username": uname, "password": "pw"})
    if r.status_code != 200:
        fail(f"setup: login failed {r.status_code}"); return user_id, None
    ...
    async with httpx.AsyncClient(base_url=base_url, timeout=60) as client, AsyncSessionLocal() as session:
        try:
            ...
            with check("saved_locations", "save: duplicate"):
                ...
                if len(rows) != 1:
                    fail(f"save dup: {len(rows)} rows, expected 1")
            
            #test save fake location
            with check("saved_locations", "save: invalid location"):
                r = await client.post("/users/me/saves", headers=header, json={"location_id": fake_loc})
                if r.status_code != 404:
                    fail(f"save invalid: status {r.status_code}, expected 404")

            #test if saved location exists for user
            with check("saved_locations", "save: save exists for user"):
                r = await client.get("/users/me/saves", headers=header)
                if r.status_code != 200:
                    fail(f"save list: status {r.status_code}")
                elif real_loc not in {s["location_id"] for s in r.json()}:
                    fail("save list: created save missing")
            
            #test delete
            with check("saved_locations", "save: delete"):
                r = await client.delete(f"/users/me/saves/{real_loc}", headers=header)
                if r.json().get("deleted") is not True:
                    fail(f"save delete: {r.json()}, expected deleted true")
                await session.commit()
                if (await session.execute(select(Saved_Location).where(
                        Saved_Location.id == save_id))).scalar_one_or_none() is not None:
                    fail("save delete: row still in DB")

            #test delete non existent saved location
            with check("saved_locations", "save: delete-noop"):
                r = await client.delete(f"/users/me/saves/{real_loc}", headers=header)
                if r.json().get("deleted") is not False:
                    fail(f"save delete-noop: {r.json()}, expected deleted false")
        except Exception as e:
            fail(f"saves cycle: EXCEPTION {type(e).__name__}: {e}")
        finally:
            await cleanup_test_users(session)
...
async def test_recent_cycle():
    async with httpx.AsyncClient(base_url=base_url, timeout=60) as client, AsyncSessionLocal() as session:
        try:
            ...
            with check("recent_locations", "recent: upsert"):
                ...
                ra2 = await client.post("/users/me/recent_locations", headers=header, json={"location_id": loc1})
                if ra2.json()["recent_id"] != recent_id_a:
                    fail(f"recent upsert: a got new recent_id {ra2.json()['recent_id']}, expected {recent_id_a}")
                await session.commit()
                rows = (await session.execute(select(Recent_Location).where(
                    Recent_Location.user_id == user_id))).scalars().all()
                if len(rows) != 2:
                    fail(f"recent upsert: {len(rows)} rows, expected 2 (a should not duplicate)")

            #get recent locations limit 5
            with check("recent_locations", "recent: order"):
                r = await client.get("/users/me/recent_locations", headers=header)
                items = r.json()
                if len(items) > 5:
                    fail(f"recent order: {len(items)} items, expected <= 5")
                if not items or items[0]["location_id"] != loc1:
                    fail(f"recent order: first is {items[0]['location_id'] if items else None}, expected {loc1}")

            #add invalid location
            with check("recent_locations", "recent: invalid"):
                r = await client.post("/users/me/recent_locations", headers=header, json={"location_id": fake_loc})
                if r.status_code != 404:
                    fail(f"recent invalid: status {r.status_code}, expected 404")

            # delete one recent loc
            with check("recent_locations", "recent: delete one"):
                r = await client.request("DELETE", "/users/me/recent_locations/one",
                                         headers=header, json={"location_id": loc1})
                if r.json().get("deleted") is not True:
                    fail(f"recent delete-one: {r.json()}, expected deleted true")
                await session.commit()
                ...
                if still is not None:
                    fail("recent delete-one: row still in DB")

            # delete all recent loc for user
            with check("recent_locations", "recent: delete all"):
                r = await client.delete("/users/me/recent_locations/all", headers=header)
                if r.json().get("deleted") is not True:
                    fail(f"recent delete-all: {r.json()}, expected deleted true")
                await session.commit()
                rows = (await session.execute(select(Recent_Location).where(
                    Recent_Location.user_id == user_id))).scalars().all()
                if rows:
                    fail(f"recent delete-all: {len(rows)} rows remain")

            #delete all again will return false
            with check("recent_locations", "recent: delete all noop"):
                r = await client.delete("/users/me/recent_locations/all", headers=header)
                if r.json().get("deleted") is not False:
                    fail(f"recent delete-all-noop: {r.json()}, expected deleted false")
        except Exception as e:
            fail(f"recent cycle: EXCEPTION {type(e).__name__}: {e}")
        finally:
            await cleanup_test_users(session)
...
async def assert_accessible_path(session, route):
    ...
    for s in route["steps"]:
        ...
        if key not in edge_acc:
            fail(f"  PHANTOM EDGE: step {s['step_number']} {key} not a real edge")
        elif edge_acc[key] is False:
            fail(f"  ACCESSIBLE VIOLATION: step {s['step_number']} traverses inaccessible edge")


async def test_route_optimality():
    async with httpx.AsyncClient(base_url=base_url, timeout=60) as client, AsyncSessionLocal() as session:
        for case in test_input.routes: 
            with check("route_optimality", f"route {case.start_id}->{case.destination_id} [{case.mode}]"):
                ...
                if not nx_has_path:
                    if r.status_code != 404:
                        fail(f"  nx: no path, but endpoint returned {r.status_code} (expected 404)")
                    continue
                if r.status_code != 200:
                    fail(f"  endpoint {r.status_code}, but a path exists (expected 200)")
                    continue
                ...
                #compare time diff
                if dur > nx_cost + 1e-3:
                    fail(f"  SUBOPTIMAL: route {dur:.3f}s > optimum {nx_cost:.3f}s "
                          f"(heuristic likely inadmissible — edge faster than {HEURISTIC_SPEED} m/s on this route)")
                elif dur < nx_cost - 1e-3:
                    fail(f"  INCONSISTENT: route {dur:.3f}s < nx optimum {nx_cost:.3f}s "
...
def render_summary():
    ...
    cs = min(max(len("SUITE"),    max(len(r.suite)    for r in RESULTS)), 16)
    cl = min(max(len("CASE"),     max(len(r.label)    for r in RESULTS)), 34)
    ce = min(max(len("EXPECTED"), max(len(r.expected) for r in RESULTS)), 38)
    ca = min(max(len("ACTUAL"),   max(len(r.actual)   for r in RESULTS)), 38)
    clip = lambda s, n: s if len(s) <= n else s[:n-1] + "…"

    head = f"{'SUITE':<{cs}}  {'CASE':<{cl}}  {'EXPECTED':<{ce}}  {'ACTUAL':<{ca}}  RESULT"
    bar = "=" * len(head)
    print(f"\n{bar}\nSUMMARY\n{bar}\n{head}\n{'-'*len(head)}")

    last = None
    for r in RESULTS:
        suite = r.suite if r.suite != last else ""
        last = r.suite
        print(f"{clip(suite,cs):<{cs}}  {clip(r.label,cl):<{cl}}  "
              f"{clip(r.expected,ce):<{ce}}  {clip(r.actual,ca):<{ca}}  "
    ...
    per = {}
    for r in RESULTS:
        p = per.setdefault(r.suite, [0, 0])
        p[0 if r.passed else 1] += 1
    for suite, (p, f) in per.items():
        print(f"{suite:<{cs}}  {p} passed, {f} failed")
    tp = sum(r.passed for r in RESULTS)
    print(f"{bar}\nTOTAL: {tp}/{len(RESULTS)} passed, {len(RESULTS)-tp} failed\n{bar}")
