...
async def update_user_save(user_id: str, location_id: str, session: AsyncSession = Depends(database.get_db_session)):
    #search data base and update
    return {}

@router.delete("/{location_id}")
async def delete_user_save(user_id: str, location_id: str, session: AsyncSession = Depends(database.get_db_session)):
    #same as add search and delete
    boolean = await save_service.delete_user_save(session, user_id, location_id)
    return {
        "deleted": boolean
    }
