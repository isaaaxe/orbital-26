...
export default function Login() {
    ...
    const [isValidInputs, setIsValidInputs] = useState({email: true, password: true, username: true})
    ...
    return <View style={styles.screen}>
        <SafeAreaView style={styles.safeArea}>
            <View style={styles.authToggleContainer}>
                <TouchableOpacity style={[styles.authToggleItem, isLogin && styles.authToggleItemSelected]} onPress={() => setIsLogin(true)}>
                    <Text style={[styles.authToggleText, isLogin && styles.authToggleTextSelected]}>Login</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.authToggleItem, !isLogin && styles.authToggleItemSelected]} onPress={() => setIsLogin(false)}>
                    <Text style={[styles.authToggleText, !isLogin && styles.authToggleTextSelected]}>Sign Up</Text>
                </TouchableOpacity>
            </View>
            ...
            <View style={styles.searchView}>
                <TextInput 
                    ...
                    onSubmitEditing={()=>{checkValidEmail(email)}}
                    autoCapitalize="none"
                />
            </View>
                {!isValidInputs.email ? <View style={styles.errorBox}><Text style={styles.errorText}>Please enter a valid email address.</Text></View> : <></>}
            
                {!isLogin ? <><View style={styles.searchView}>
                <TextInput 
                    style={styles.searchInput}   
                ...
                {!isValidInputs.username ? <View style={styles.errorBox}><Text style={styles.errorText}>Please enter a username longer than 3 characters.</Text></View> : <></> }
                </>
                :<></>}
                <View style={styles.searchView}>
                <TextInput 
            ...
            <StylisedButton buttonText={isLogin ? "Login" : "Sign Up"} onPress={isLogin ? () => handleLogin(email, password) : () => handleSignup(email, password, username)}/>
            </View>
        </SafeAreaView>
    </View>
}